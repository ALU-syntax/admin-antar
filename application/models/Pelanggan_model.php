...
class Pelanggan_model extends CI_model
{
    ...
    {
        $this->db->where('id', 1);
        $query = $this->db->get('app_settings')->row();
        return $query->fcm_key;
    }
    ...
    public function fiturhome($idcabang)
    {
        $this->db->select('*');
        $this->db->from('fitur');
        ...
        foreach($query as $key)
        {
            $wil = $key['wilayah'];
            $arrwil = explode(',', $wil);
            for($i=0; $i<count($arrwil); $i++)
            {
                if($idcabang == $arrwil[$i]) 
                {
                    $row['id_fitur'] = $key['id_fitur'];
                    $row['fitur'] = $key['fitur'];
                    $row['biaya'] = $key['biaya'];
                    $row['biaya_minimum'] = $key['biaya_minimum'];
                    $row['jarak_minimum'] = $key['jarak_minimum'];
                    $row['maks_distance'] = $key['maks_distance'];
                    $row['wallet_minimum'] = $key['wallet_minimum'];
                    $row['komisi'] = $key['komisi'];
                    $row['keterangan_biaya'] = $key['keterangan_biaya'];
                    $row['driver_job'] = $key['driver_job'];
                    $row['keterangan'] = $key['keterangan'];
                    $row['icon'] = $key['icon'];
                    $row['home'] = $key['home'];
                    $row['active'] = $key['active'];
                    $row['fee_driver'] = $key['fee_driver'];
                    $row['id_provinsi'] = $key['id_provinsi'];
                    $row['id_kabupaten'] = $key['id_kabupaten'];
                    $row['wilayah'] = $key['wilayah'];
                    array_push($data, $row);
                }
            }
    ...
    public function fiturhomeall($idcabang)
    {
    ...
    public function get_driver_ride($lat, $lng, $fitur)
    {
        ...
        if($drj == '8')
        {
            $result = $this->db->query("
                SELECT f.jarak_minimum, f.wallet_minimum, f.wilayah, d.id as id, d.nama_driver, ld.latitude, ld.longitude, ld.bearing, ld.update_at,
                ...
                WHERE ld.id_driver = d.id 
                    ...
                    AND ld.status = '1'
                    AND dj.id = 8
                    AND (d.job = 8 OR d.job = 23) 
                    AND d.wilayah IN ($wil)
                    AND d.status = '1'
...
<!--[if (mso)|(IE)]></td></tr></table><![endif]-->
    ...
    public function insert_data_transaksi_merchant($data_transaksi, $dataDetail, $total_belanja)
    {
        ...
        $transaksi = [
            ...
            'wilayah_pelanggan' => $data_transaksi['wilayah_pelanggan']
        ];
    ...
    {
        $this->db->select('transaksi_item.id_merchant, transaksi_item.jumlah_item,item.nama_item, transaksi_item.catatan_item, transaksi_item.total_harga');
    ...
    public function buat_master_item($pes)
    {
        $data = array(
            "id_merchant" => 9999,
            "nama_item"=> $pes->nama_item,
            "harga_item" =>$pes->total_harga,
            "harga_promo"=>$pes->total_harga,
            "kategori_item"=>283,
    ...
    public function update_nama_merchant($nama_resto)
    {
        $this->db->where('id_merchant', 9999);
        $this->db->update('merchant', array("nama_merchant"=> $nama_resto));
    }
    
    public function Hitungpoin($id)
    {
        $query = $this->db->query("SELECT COUNT(id) as POIN FROM point_transaction WHERE driver_id = '$id'");
        return $query->result();
    }
    
    public function poinpelanggan($id) 
    {
        $query = $this->db->query("SELECT COUNT(b.id_pelanggan) AS poin_pelanggan FROM point_transaction a LEFT JOIN transaksi b ON a.trans_id = b.id WHERE b.id_pelanggan = '$id'");
        return $query->result();
        
    }
    
    
    public function sendlocation($id_pelanggan, $lat, $long)
    {
       
        $this->db->where('id', $id_pelanggan);
        $query = $this->db->update('pelanggan', array(
            "lat_position" => $lat,
            "long_position" => $long,
            "date_position" => date('Y-m-d H:i:s')
        )) ;
    ...
    function get_customer_location_admin()
    {
        $this->db->select(''
            . 'pelanggan.id,'
            . 'pelanggan.lat_position,'
            . 'pelanggan.long_position,'
            . 'pelanggan.status,'
            . 'pelanggan.fullnama');
        $this->db->where('pelanggan.lat_position != "" ');
        $this->db->where('pelanggan.long_position != "" ');
        $loc = $this->db->get('pelanggan');
        return $loc;
    }

    
    
    public function referalprocess($refcode) 
    {
        if(!empty($refcode)) 
        {
            
            $jumlah_bonus = $this->db->get('app_settings')->result();
            $jumlah = $jumlah_bonus[0]->ref_charge;
            
            $this->db->where('referal_code', $refcode);
            $query = $this->db->get('pelanggan');
            
            if($query->num_rows() == 1) {
                $data = $query->result();
                
                $data_insert_wallet = array(
                    "id_user" => $data[0]->id,
                    "jumlah" => $jumlah,
                    "bank" => "bonus referal",
                    "nama_pemilik" => $data[0]->fullnama,
                    "rekening" => "admin",
                ...
                $this->db->insert("wallet", $data_insert_wallet);
                
                $id_user = $data[0]->id;
                $this->db->query("UPDATE saldo SET saldo = saldo + $jumlah WHERE id_user = '$id_user'");
            }
            else
            {
                $this->db->where('referal_code', $refcode);
                ...
                if($query->num_rows() == 1) 
                {
                    ...
                    $data_insert_wallet = array(
                        ...
                        "jumlah" => $jumlah,
                        "bank" => "bonus referal",
                        "nama_pemilik" => $data[0]->nama_driver,
                        "rekening" => "admin",
                        "waktu" => date('Y-m-d H:i:s'),
                        "type" => "topup",
                        "status" => 1
                ...
                else 
                {
                    ...
                    if($query->num_rows() == 1) 
                    {
                        $data = $query->result();
                        $data_insert_wallet = array(
                            "id_user" => $data[0]->id_mitra,
                            "jumlah" => $jumlah,
                            "bank" => "bonus referal",
                            "nama_pemilik" => $data[0]->nama_mitra,
                            "rekening" => "admin",
                            "waktu" => date('Y-m-d H:i:s'),
                            "type" => "topup",
                            "status" => 1
